...
import { Eye, EyeOff, ArrowRight, AlertCircle, HelpCircle, Lock, X } from 'lucide-react';
...
export default function AddSitePage() {
  ...
  if (selectedSite) {
    return (
      <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
        ...
        <div className="relative w-full max-w-md mx-0 sm:mx-4 bg-white rounded-t-2xl sm:rounded-2xl border border-surface-3 overflow-hidden shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-surface-3 sticky top-0 bg-white z-10">
            <button
              onClick={handleBack}
            ...
            <button onClick={handleClose} className="p-1 rounded-lg hover:bg-surface-2 text-gray-400">
              <X className="w-4 h-4" />
            </button>
          </div>
                ...
                {showHelp && (
                  <div className="mt-2 p-3 rounded-lg bg-surface-1 border border-surface-3 text-xs text-gray-500 leading-relaxed space-y-1.5">
                    ...
                    <ol className="list-decimal list-inside space-y-1 text-gray-500">
                      ...
                      <li>Go to <span className="text-gray-700">Users → Profile</span></li>
  ...
  // ── Step 1: Site picker (bottom sheet) ──────────────────────
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
      <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={handleClose} />
      <div className="relative w-full max-w-md mx-0 sm:mx-4 bg-white rounded-t-2xl sm:rounded-2xl border border-surface-3 overflow-hidden shadow-2xl animate-slide-up max-h-[85vh]">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-surface-3 sticky top-0 bg-white z-10">
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Add a Site</h2>
            <p className="text-xs text-gray-400 mt-0.5">Select the site you want to connect</p>
          </div>
          <button onClick={handleClose} className="p-1 rounded-lg hover:bg-surface-2 text-gray-400">
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="overflow-y-auto max-h-[calc(85vh-72px)]">
          {Object.entries(grouped).map(([catKey, catSites]) => (
            <div key={catKey} className="pt-3 px-3">
                ...
                {catSites.map((site) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isConnected ? (
                        <span className="text-[10px] text-green-600 font-medium px-2 py-0.5 bg-green-50 rounded-full">Connected</span>
                      ) : (
